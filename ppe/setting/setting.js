      ...
      TreeService.getTree().then(function (result) {      
        ...
        socket.emit('web-setting-get-data', { base: 'serverRole', query: {}}, function(err, retRole)  {
          socket.emit('web-setting-get-data', { base: 'employee', query: {_id: '0000'} }, function(err, retEmp)  {
            socket.emit('web-setting-get-data', { base: 'user', query: {}}, function(err, retPat)  { 
              socket.emit('web-setting-get-data', { base: 'drug', query: {}}, function(err, retDru)  { 
//------------------------------------------------------------------------------------------------
              console.log(err, retPat);
              patientList = retPat;
              $scope.patientList = patientList;

              console.log(err, retDru);
              drugList = retDru;
              $scope.drugList = drugList;
//---------------------------------------------------------------------------------------------
  ...
  /*----------------------------------------------------------------แก้ไขครั้งที่1-----------------------------------------------------------------------------*/
    //           console.log (retPat[0].patient);
  
    //             /*historymedicineList*/
    // /****************************แก้ไข ครั้งที่2************************************************/
  
    //           patientList = retPat[0].patient;
    //           $scope.patientList = patientList;
    // /****************************แก้ไข ครั้งที่2************************************************/
  ...
  /*----------------------------------------------------------------แก้ไขครั้งที่1-----------------------------------------------------------------------------*/
    ...
    // socket.emit('web-get-data-patient',function(data){
    //   console.log(data);
    // $timeout(function() {
    ...
    $scope.saveEmployee = function(ev)  {
      ...
      for (var i in employeeList)  {
        if (employeeList[i].fullname == '' || employeeList[i].lastname == '')  {
          $mdDialog.show(
            $mdDialog.alert()
            .title('Alert')
            .textContent('Required field "Full-name" and "Last-name" please check and try again.')
            .targetEvent(ev)
    ...
    $scope.savePatient = function(ev)  {      
      ...
      $mdDialog.show(confirm).then(function() {
        // console.log(counterList);    
         success = true;
        // alert(count+'/'+success);    
        retrySavePatient(200, ev);
  
      }, function() {
      });
  
    }
    var retrySavePatient = function (retryInMilliseconds, ev) {
      $timeout(function(){
        // alert('Sucess = '+success);
        if (!success) {
          retrySavePatient(200);
        }
        else  {
          var obj = $scope.patientList;
          socket.emit('web-setting-write-patient', { base: 'patient', data: obj}, function(err, ret)  {
  ...
  $scope.saveDrug = function(ev)  {

    var obj = $scope.drugList;
    // console.log(obj);
    socket.emit('web-setting-write-drug', { base: 'drug', data: obj}, function(err, ret)  {
      $mdDialog.show(
        $mdDialog.alert()
        .title('Alert')
        .textContent(ret.text)
  ...
  /*----------------------------------------------------------------แก้ไขครั้งที่3-----------------------------------------------------------------------------*/
    ...
    //      var obj = { patient: historymedicineList };
    //       socket.emit('web-setting-write-data', { base: 'employee', data: obj, flag: 0 }, function(err, ret)  {
    ...
    //       });
    //        console.log(historymedicineList);
    //  }
      /****************************แก้ไข ครั้งที่2************************************************/
    ...
    /*$scope.addpatient = function(patient)  {
      patient.push({name: '', surname: '',age: '',bloodtype: '',sex: '',namemedicine:'',tel: ''});
    }
